...
function dbc2array($filename, $format)
{
	...
	$unpack_fmt = array('x'=>"x/x/x/x", 'X'=>"x", 's'=>"V", 'f'=>"f", 'i'=>"V", 'I' => 'l', 'b'=>"C", 'd'=>"x4", 'n'=>"V");
	$unpackstr = "";
...
{
	exit("you should specify location as first argument\n");
}

define('DBC_LOCATION', $_SERVER['argv'][1]);

//Парсим локации:
print("INSERT INTO site_zones (id, name) VALUES\n");
$data = dbc2array(DBC_LOCATION.'/AreaTable.dbc', 'nxxxxxxxxxxxxxxxxxxsxxxxxxxxxxxxxxxx');
$data = array_map(create_function('$item', 'return "(".$item[0].", \'".addslashes($item[1])."\')";'), $data);
print(implode(",\n", $data).";\n\n");

//Парсим достижения:
print("INSERT INTO site_achvs (id, category) VALUES\n");
$data = dbc2array(DBC_LOCATION.'/Achievement.dbc', 'nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxIxxxxxxxxxxxxxxxxxxxxxxx');
$data = array_map(create_function('$item', 'return "(".$item[0].", ".$item[1].")";'), $data);
print(implode(",\n", $data).";\n\n");

//Парсим категории достижений:
print("INSERT INTO site_achv_categories (id, category) VALUES\n");
$data = dbc2array(DBC_LOCATION.'/Achievement_Category.dbc', 'nIxxxxxxxxxxxxxxxxxx');
$data = array_map(create_function('$item', 'return "(".$item[0].", ".$item[1].")";'), $data);
print(implode(",\n", $data).";\n\n");
